...
export class PaymentsService {
	...
	async createCharge({
		...
		email,
		reservationId,
	}: CreateChargeDto & { email: string; reservationId: string }) {
			...
			case 'checkout.session.completed':
				{
					...
					const reservationId =
						...
						'07605416-0dd9-4034-8910-132fe14a8be2';
					const email = session.metadata?.email || 'john.doe@example.com';

					this.logger.log(
						`Payment completed for reservation: ${reservationId}`
					);

					if (!(reservationId && email)) {
						this.logger.error(
							'Missing reservation ID or email in webhook metadata'
						);
						break;
					}
					...
					try {
						...
						const signedMessage = this.configService.createSignedMessage(
							payload,
							'payments'
						);

						await firstValueFrom(
							this.reservationsService.send(
						...
						this.logger.log(
							`Reservation ${reservationId} status updated to PENDING_APPROVAL`
						);
			...
			case 'checkout.session.expired':
				{
					...
					try {
						...
						this.logger.log(
							`Reservation ${reservationId} status updated to CANCELED due to payment expiration`
						);
