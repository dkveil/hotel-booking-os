...
export class ReservationsService {
	...
	): Promise<{ data: Reservation[]; pagination: Pagination }> {
		const {
			page,
			limit,
			order,
	...
	async update(
		...
		additionalFilter: Partial<Reservation> = {}
	): Promise<Reservation> {
		const reservation = await this.reservationsRepository.transaction(
			async (db: DatabaseService) => {
				const whereClause = {
					id,
					...additionalFilter,
				...
				if (status) updateData.status = status as ReservationStatus;
	...
	async updateWithEmailVerification(
		...
		const user = await this.reservationsRepository.transaction(
			async (db: DatabaseService) => {
	...
	private async addToUserReservationsList(
		userCacheKey: string,
